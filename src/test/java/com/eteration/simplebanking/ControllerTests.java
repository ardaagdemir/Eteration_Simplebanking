...
class ControllerTests  {
    ...
    @Test
    public void create_Account_withCreditAndThenDebit_thenReturnJson(){
        Account account = new Account("Jim", "12345");
        account.post(new DepositTransaction(1000d));
        account.post(new WithdrawalTransaction(200d));
        //account.post(new PhoneBillPaymentTransaction("Vodafone", "5423345566", 96.50));
        assertEquals(account.getBalance(), 800.0, 0.0001);
    }
    @Test
    ...
    throws Exception {
        ...
        doReturn(account).when(accountService).findAccount( "17892");
        ResponseEntity<TransactionStatus> result = controller.credit( "17892", new DepositTransaction(1000.0));
        verify(accountService, times(1)).findAccount("17892");
    ...
    throws Exception {
        ...
        verify(accountService, times(2)).findAccount("17892");
        assertEquals("OK", result.getBody().getStatus());
